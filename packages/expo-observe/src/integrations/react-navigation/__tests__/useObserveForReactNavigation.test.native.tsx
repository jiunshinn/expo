...
describe('useObserveForReactNavigation', () => {
  ...
  it('skips markInteractive when not focused, but still computes TTI', async () => {
  ...
  it('throws when isInitialized() flips mid-lifetime', () => {
    ...
    expect(() => rerender(undefined)).toThrow(
      "[expo-observe] React Navigation integration was toggled during a screen's lifecycle. Call `ExpoObserve.configure({ disableReactNavigationIntegration })` once at startup before any screen mounts."
    );
  });
  ...
  it('uses the stashed pathname from screenTimes (matching the matching TTR routeName)', async () => {
    storage.screenTimes['screen-a'] = { dispatchTime: 1000, pathname: '/parent/A?foo=bar' };
    jest.spyOn(performance, 'now').mockReturnValue(1300);
