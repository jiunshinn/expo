...
describe('ObserveNavigationContainer', () => {
  ...
  it("passes a getPathname that falls back to '/' + route.name when no linking config is supplied", () => {
    render(
      <ObserveNavigationContainer>
        <Text>child</Text>
      </ObserveNavigationContainer>
    ...
    const [, getPathname] = createStateChangeHandlerMock.mock.calls[0];
    expect(getPathname(undefined, { name: 'Profile' })).toBe('/Profile');
  });
  ...
  it('throws when isInitialized() flips during the container lifetime', () => {
    ...
    expect(() =>
      ...
      '[expo-observe] React Navigation integration was enabled after application mounted. Call ExpoObserve.configure() before rendering ObserveNavigationContainer.'
    );
  });
});
